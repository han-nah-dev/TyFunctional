...
A fork of [PyFunctional](https://github.com/EntilZha/PyFunctional), adding return types to chained functions. Now, the result of a chained operator `seq(1, 2, 3).map(lambda x: MyObj(x)).to_list()` is not just `Iterable | tuple | Any`, but actually `list[MyObj]`.
