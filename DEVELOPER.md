...
3. Wait for tests to pass on [GitHub Actions](https://github.com/han-nah-dev/TyFunctional/actions)
4. Wait for coverage to complete at 100% on [Coveralls](https://coveralls.io/github/han-nah-dev/TyFunctional)
5. Wait for docs to complete building successfully at the [TyFunctional repository](https://github.com/han-nah-dev/TyFunctional)

### Testing Local Release
1. Run `docker run -it python bash` for clean python installation
2. Clone and install `TyFunctional` with `python setup.py install`
3. Run a python terminal session and insure that `import tyfunctional` returns with no errors
...
1. Run `twine upload -r test dist/*` to upload `TyFunctional` to the test PyPi server
...
3. Run `docker run -it python bash` and install the package using `pip install -i https://testpypi.python.org/pypi tyfunctional`.
4. Install dependencies not on the test PyPI instance: `future`, `six`, `dill`, and `backports.lzma`
5. Test that `tyfunctional` is importable
...
1. Run `twine upload -r pypi dist/*` to publish `TyFunctional` to the live PyPi server.
